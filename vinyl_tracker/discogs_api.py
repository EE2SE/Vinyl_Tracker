...
class DiscogsAPI:
    def __init__(self):
        load_dotenv()
        self.__base_price_url = (
            "https://api.discogs.com/marketplace/price_suggestions/{}"
        )
        self.__pat = os.getenv("DISCOGS_PAT") or self._raise_env_error("DISCOGS_PAT")
        self.__key = os.getenv("CONSUMER_KEY_DISCOGS") or self._raise_env_error(
        ...
        self.__access_token_url = os.getenv(
            "ACCESS_TOKEN_URL_DISCOGS"
        ) or self._raise_env_error("ACCESS_TOKEN_URL_DISCOGS")
        self.client = None
